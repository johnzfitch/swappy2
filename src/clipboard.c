...
static gboolean send_pixbuf_to_wl_copy(gdk_pixbuf_t *pixbuf) {
  ...
  // Use maximum compression for clipboard copy (lossless, smaller size)
  gdk_pixbuf_save_to_buffer(pixbuf, &buffer, &size, "png", &error,
                            "compression", "9", NULL);
...
static void show_copy_notification(const char *message) {
  pid_t pid = fork();
  if (pid == 0) {
    execlp("notify-send", "notify-send", "-t", "2000", "-a", "Swappy",
           "Copied to Clipboard", message, NULL);
    _exit(1);
  }
}
...
bool clipboard_copy_drawing_area_to_selection(struct swappy_state *state) {
  ...
  if (send_pixbuf_to_wl_copy(pixbuf)) {
    ...
    success = TRUE;  // Assume success for GDK fallback
  }
  ...
  if (success) {
    ...
    char msg[128];
    g_snprintf(msg, sizeof(msg), "Image %dx%d copied", width, height);
    show_copy_notification(msg);
  }
